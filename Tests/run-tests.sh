...
clang++ -std=c++17 -O2 "$TASK_TEST_DEFINE" -DJUCE_GLOBAL_MODULE_SETTINGS_INCLUDED=1 \
    ...
    -I JuceLibraryCode -I ../JUCE/modules Tests/VoiceEngineTests.cpp \
    ...
    -framework Metal -framework MetalKit -o "$TASK_TEST_DIR/voice-tests"
if [ "$#" -ge 2 ]; then
    "$TASK_TEST_DIR/voice-tests" "$2"
else
    "$TASK_TEST_DIR/voice-tests"
fi
